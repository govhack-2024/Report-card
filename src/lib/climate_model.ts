...
export function predictForwardYears(
  data: LocationRiseResponse,
  years: number,
): SeaLevelPoint {
  let vml_change = (data.vlm_estimation.rate_mm_per_year * years) / 1000;
...
export function predictFindIntercept(
  data: LocationRiseResponse,
): SeaLevelPoints {
  ...
  for (let year = 0; year < search_max; year++) {
    ...
    if (
      relative_sea_level + data.tide_estimation.spring_tide_min >
      target_elv
    ) {
    ...
    if (
      ...
      relative_sea_level + data.tide_estimation.surge_tide_max >
      target_elv
    ) {
